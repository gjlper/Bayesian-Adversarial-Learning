...
#  --- White-box attacks ---
class CertAttack(object):
    def __init__(self, model=None, epsilon=0.5, k=15, a=1, gamma=0.04, sigma=0.5, beta=0.9, skinit=1.0, order='inf',
        random_start=False, projection=False):
        """
        Certifying some distributional robustness adversarial training
        """
        self.model = model
        ...
        self.skinit = skinit
        self.order = order
        print('Init Cert Attack')
        self.projection = projection
        if projection:
            print('use norm 2 projection to restrict the value range')
    ...
    def perturb(self, X_nat, y):
        ...
        #Implement the k step Armijo's Rule for optimization
        ...
        X_var0 = X_var.clone()
        loss = loss - self.gamma*torch.sum((X_var-X_var0)**2)
        loss.backward()
        grad = X_var.grad.data.cpu().numpy()
        
        objold = loss.data.cpu().numpy()
        sigma = self.sigma
        beta = self.beta
       
        for i in range(self.k):
            flag = False
            #sk = 1.0/np.max(grad)*self.skinit
            sk = 1.0*self.skinit
            while (flag!=True):
                ...
                #X_new = X + sk*grad
                X_new = X + sk*normalized_grad
                X_new_var = to_var(torch.from_numpy(X_new))
                scores = self.model(X_new_var)
                #print(X_new_var)
                #print('loss formulation', self.loss_fn(scores, y_var),  self.gamma*torch.sum((X_new_var-X_var0)**2))
                #exit(0)
                objnew = self.loss_fn(scores, y_var) - self.gamma*torch.sum((X_new_var-X_var0)**2)

                objnew = objnew.data.cpu().numpy()
                #print('objnew - objold', objnew-objold)
                #print(grad.shape)
                #print(X.shape)
                #if (objnew-objold) <= sigma*np.transpose(grad)*(X_new-X):

                if (objnew-objold)[0] >= sigma*np.inner(grad.flatten(), (X_new-X).flatten()):
                    flag = True
                    break
                else:
                    sk = sk*beta
        ...
        #print('maximum diff adv', np.max(np.abs(X_new-X_nat)))
        #print('maximum grad adv', np.max(np.abs(grad)))

        if self.projection == True:
            normalized_d = (X_new - X_nat)/np.linalg.norm(X_new-X_nat)
            X_new = X_nat + normalized_d*self.epsilon
        return X_new

class WRMAttack(object):
    def __init__(self, model=None, eps=0.3, steps=15, storeadv=False):
        #WRM implementation based on https://github.com/duchi-lab/certifiable-distributional-robustness/blob/master/attacks_tf.py
        ...
        print('Use WRM Adversarial training')
    ...
    def perturb(self, X_nat, y):
        ...
        for t in range(self.steps):
            ...
            #add wassestein distance
            loss = self.eps*loss - torch.sum((X_var-X_var0)**2) 
            loss.backward()
            grad = X_var.grad.data.cpu().numpy()
        ...
        if self.storeadv == True:
            X = np.clip(X, 0, 1)*255
            cv2.imwrite(os.path.join(advtrainfolder, 'WRM_Advtraining.png'), np.squeeze(X[0,:]))
            exit(0)
